...
simparams = SimParams(Csh=1, # Coefficient of sh energy in psi evolution
                      Cw          = 0.1,          # Coefficient of penalty term in psi evolution
...
filename  = "Single"+str(simparams.dt)+"_"+str(simparams.Cw)+"_"+str(scale)
# filename  = "debugRes"+str(simparams.dt)+"_"+str(simparams.Cw)+"_"+str(scale)
path = "./out/single/" # outputpath
...
yp = geometry.H/2
xp1 = geometry.L/2 #(geometry.L/(geometry.Nx+1))*(4*(geometry.Nx)//10-7)
