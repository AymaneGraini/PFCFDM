...
figsh, ax = plt.subplots(figsize=(4.5, 4))
axins = inset_axes(ax, width="60%", height="30%", 
...
for i, cw in enumerate(cws):
    dataSH = np.loadtxt("out/Static/nonperiodic/penaltyMec/SG_energy_testshit delitee0.2_"+str(cw)+".csv",delimiter="\t")
    dt = dataSH[1,0] - dataSH[0,0]
    ax.plot(dataSH[:,0]/dt,dataSH[:,1]-dataSH[0,1],lw=0.9,label=r"$C_w={{{}}}$".format(cw),color=colors[i])

    datauq = np.loadtxt("out/Static/nonperiodic/penaltyMec/errors_testshit delitee0.2_"+str(cw)+".csv",delimiter="\t")
    l2 = np.sqrt(datauq[:,1]**2 +datauq[:,2]**2 +datauq[:,3]**2 +datauq[:,4]**2)
    ax2.plot(datauq[:,0]/dt,l2,lw=0.9,label=r"$C_w={{{}}}$".format(cw),color=colors[i])
...
ax.set_xlabel(r"Iterations, $dt={{{}}} \, [-]$".format(dt),fontsize=14)    
ax.set_ylabel(r"Swift Hohenberg Energy $\mathcal{F}_{sh} -\mathcal{F}_0$",fontsize=14)
...
ax12.set_ylabel(r"$||\mathbf{U^e_{12}}-\mathbf{Q_{12}}||_{L_2}$",fontsize=14)   

ax21.set_xlabel(r"Iterations, $dt={{{}}} \, [-]$".format(dt),fontsize=14)    
ax21.set_ylabel(r"$||\mathbf{U^e_{21}}-\mathbf{Q_{21}}||_{L_2}$",fontsize=14) 
...
ax.set_xlim(None,ylm)
ax.axvspan(0, 1000, color='royalblue', alpha=0.1) 
...
ax.axvline(x=1000,lw=0.5,color="black",ls="dashed")
axins.axvline(x=1000,lw=0.5,color="black",ls="dashed")
ax.text(350,-75,r"Relaxtion of $\psi$ with $\mathcal{F}_{sh}$ only",rotation=90, va='center', ha='center')
...
# figsh.savefig("Swifth-Hohenberg energy.png",dpi=300,bbox_inches='tight')
# figuq.savefig("UQ difference.png",dpi=300,bbox_inches='tight')
# figuq11.savefig("UQ11 difference.png",dpi=300,bbox_inches='tight')
# figuq12.savefig("UQ12 difference.png",dpi=300,bbox_inches='tight')
# figuq21.savefig("UQ21 difference.png",dpi=300,bbox_inches='tight')
# figuq22.savefig("UQ22 difference.png",dpi=300,bbox_inches='tight')
plt.show()
