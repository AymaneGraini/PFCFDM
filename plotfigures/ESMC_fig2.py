...
X,Y,psi,t,allts = get_im_data(path,"alpha",0,2)

fig2, ax2 = plt.subplots(figsize=(6,9))
cont = ax2.contourf(X, Y, psi, levels=200, cmap="Blues")

# add the colorbar
cbar = fig2.colorbar(cont, ax=ax2, pad=0.02)   # pad tweaks spacing
cbar.set_label(r"$\alpha^{13}$",fontsize=14)                      # optional label
plt.show()
fig2.savefig("./figs/esmc/alpha.png",dpi=300,bbox_inches="tight")
exit()
...
for cw in cws :
    ...
    X,Y,UE11,tt,allts = get_im_data(path,"UE",t,0)
    ...
    # ax2.plot(xvals[:len(xvals)//2],val_theory[:len(xvals)//2],lw=0.9,label=r"theor",color="black")
