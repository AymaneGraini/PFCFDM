...
X,Y,UE11,t,allts = get_im_data(path,"sigmaUe",0,0)
print(X.shape)
# exit()
L=X.max()
H=Y.max()
...
def _T11(x,y):
    x1 = x
    x2 = y
    return -D *x2*(-2*(x2)**2/(x1**2+x2**2)**2+3/(x1**2+x2**2))

T11=_T11(X-L/2,Y-H/2)/mu

    
for cw in cws :
    path = "out/ESMC/Single0.1_"+str(cw)+".h5"
    X,Y,UE11,tt,allts = get_im_data(path,"sigmaUe",t,0)
    # X,Y,Q11,tt,allts = get_im_data(path,"Q",t,0)
    xvals = Y[:,center_x_index]-H/2
    print("xat taht ", X[:,center_x_index])
    print(xvals[len(xvals)//2:])
    val_UE = UE11[:,center_x_index]
    val_theory = epsxx[:,center_x_index]
    val_th = T11[:,center_x_index]
    ...
    # log_y_eps = np.log10(val_th[:len(xvals)//2])
    # ax2.scatter(log_x,log_y_eps,marker="s",edgecolors="black",facecolor="none",label=r"Litt.",alpha=0.1)

    # ax2.axvline(np.log10(a0/2),lw=0.9,ls="dashed")



    ax2.plot(xvals[:len(xvals)//2],val_UE[:len(xvals)//2],lw=0.9,label=r"UE11",color="red")
    ax2.plot(xvals[:len(xvals)//2],val_th[:len(xvals)//2],lw=0.9,label=r"UE11",color="blue")

    # ax2.plot(xvals/a0,val_UE,lw=0.9,label=r"$\mathbf{U_e}^{{11}}$",color="red")
    # ax2.scatter(xvals/a0,val_th,marker="s",edgecolors="black",facecolor="none",label=r"Hirth",alpha=0.2)
    # # ax2.axvline(0)
    # ax2.set_xlim(-10,10)
...
ax2.set_xlabel(r"$\log d$",fontsize=12,fontweight='bold')
ax2.set_ylabel(r"$\log$ deformation",fontsize=12,fontweight='bold')
# ax2.set_xlim(0.1,None)

# ax2.legend(frameon=False)
# ax2.text(1.4,      # mid‑ish x, nudged right
#         -1.7,             # geometric‑mean y
#         r'$\propto\;1/r$',
...
# fig2.savefig("./figs/esmc/U11.png",dpi=300,bbox_inches="tight")
